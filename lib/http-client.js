...
var HttpClient = function HttpClient(options) {
  ...
  if (pool) {
    var protocol = http;
    if ('ssl' in this._defaults && this._defaults['ssl'] === true) {
        protocol = https;
    }

    this._defaults._pool = new Pool(protocol, pool.servers, pool.options || {});
  }
}
...
HttpClient.prototype._execute = function(meta) {
  ...
  new HttpRequest(protocol, this, meta).execute();
}
