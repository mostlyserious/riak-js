...
HttpMeta.prototype.loadResponse = function(response) {
  ...
  if (this.transferEncoding == 'chunked' && this.contentType) this.boundary = getMultipartBoundary(this.contentType);
 ...
 * @api private
 */
HttpMeta.prototype._parseMultipartMixed = function(data, boundary) {
  var escape = function(s) { s.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&") };
  
  data = data.split(new RegExp("\r?\n--" + escape(this.boundary) + "--\r?\n"));
  data = (data && data[0]) || "";
