...
HttpRequest.prototype.execute = function() {
  ...
  this.instrumenter.emit('request.start', this.event)
  ...
  var request = http.request(meta, function(response) {
    ...
    response.on('end', function() {
      ...
      self.instrumenter.emit('request.finished', this.event);
      callback(err, buffer, meta);
    });
  });
  
  request.on('error', function(err) {
    self.instrumenter.emit('request.error', this.event)
...
HttpRequest.prototype.handleRequestEnd = function(buffer) {
  ...
  this.instrumenter.emit('request.end', this.event)
