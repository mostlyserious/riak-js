...
var HttpSearchClient = function HttpSearchClient(options, callback) {
  ...
  if (!('resource' in this._defaults)) {
    this._defaults['resource'] = 'solr';
  }
  if (!('method' in this._defaults)) {
    this._defaults['method'] = 'get';
  }
}
...
HttpSearchClient.prototype._run = function(meta) {
  ...
  meta.callback = function(err, data, meta) {
    callback(err, err === null ? data.response : null, meta);
  }
  this._execute(meta);
}
