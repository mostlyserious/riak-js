...
HttpClient = function(options) {
  this.defaults = {
    ...
    interface: 'riak',
    headers: {
      'Host': (this.options == undefined ? undefined : this.options.host) || 'localhost'
    },
    debug: true,
    callback: __bind(function(response, meta) {
      return this.log(response, {
        json: (typeof meta === "undefined" || meta == undefined ? undefined : meta.contentType) === 'application/json'
      });
    }, this)
  };
...
HttpClient.prototype.save = function(bucket, key, data, options) {
  return __bind(function(callback) {
    ...
    data = _a[0];
    options = _a[1];
    !options.method ? (options.method = key ? 'PUT' : 'POST') : null;
    meta = new Meta(bucket, key, options);
    meta.content = {
...
HttpClient.prototype.execute = function(meta) {
  return __bind(function(callback) {
    ...
    options.headers['X-Riak-Vclock'] ? (options.headers['X-Riak-ClientId'] = options.clientId) : null;
    this.log(("" + (options.method.toUpperCase()) + " " + (path)));
    (typeof meta === "undefined" || meta == undefined ? undefined : meta.content == undefined ? undefined : meta.content.value) ? (options.headers.Connection = 'close') : null;
    return this.pool.request(options.method.toUpperCase(), path, options.headers, __bind(function(request) {
      var buffer;
      (typeof meta === "undefined" || meta == undefined ? undefined : meta.content == undefined ? undefined : meta.content.value) ? request.write(meta.content.value, meta.contentEncoding) : null;
      buffer = new String();
