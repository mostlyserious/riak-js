...
Riak.prototype.execute = function (url, options) {
  ...
  return function(callback) {
    ...
    if (isArray(options.links)) {
      ...
      options.headers.link = Meta.prototype.makeLinks(options.links) + hl;
    }
    ...
    // required header by RFC2616, HTTP/1.1
...
Meta.prototype.links = function() {
  return stringToLinks(this.headers.link)
}

function stringToLinks(links) {
  var result = [];
  if (links) {
